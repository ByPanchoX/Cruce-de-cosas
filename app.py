...
st.title("📊 Consolidador 2025 + Aportes Patronales")
...
if f_lib or f_inf or f_pat:
    if st.button(f"➕ Procesar {nombre_entidad}"):
        try:
            ...
            # 3. PROCESAR APORTE PATRONAL (Nuevas columnas solicitadas)
            ...
            if not pat_data.empty:
                llave_merge = 'key' if 'key' in df_merged.columns else ('Rut' if 'Rut' in df_merged.columns else None)
                if llave_merge:
                    df_merged = pd.merge(df_merged, pat_data, left_on=llave_merge, right_on='key_rut', how='outer')
                else:
                    df_merged = pat_data

            # Limpiar llaves y añadir metadatos
            ...
            st.success(f"✅ {nombre_entidad} procesada.")
            
        except Exception as e:
            st.error(f"Error en el proceso: {e}")

# --- GENERACIÓN FINAL (FUSIONADA) ---
if st.session_state['db']:
    st.divider()
    if st.button("🚀 GENERAR EXCEL CONSOLIDADO FINAL", type="primary"):
        df_all = pd.concat(st.session_state['db'], ignore_index=True)
        
        # FUSIONAR FILAS: Agrupar por el RUT ('key') y tomar el primer valor no nulo
        df_final = df_all.groupby('key').first().reset_index()
        
        # Limpieza de nombres y columnas técnicas
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df_final.to_excel(writer, index=False, sheet_name='Consolidado_Auditoria')
        
        st.download_button(
            label="📥 Descargar Consolidado con Aportes Patronales",
            data=output.getvalue(),
            file_name=f"Consolidado_Auditoria_{mes}_2025.xlsx",
